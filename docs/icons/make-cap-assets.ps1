# 生成 Capacitor 需要的图标/启动图源文件：assets/icon-1024.png + assets/splash-2732.png
Add-Type -AssemblyName System.Drawing
$ErrorActionPreference = 'Stop'
$assetsDir = 'E:\projects\html\shiye\assets'
...
public static class CapAssets {
  ...
  public static Bitmap Splash(int size, int inner) {
    Bitmap bmp = new Bitmap(size, size);
    Graphics g = Graphics.FromImage(bmp);
    g.SmoothingMode = SmoothingMode.AntiAlias;
    g.Clear(Color.FromArgb(255, 247, 244, 236));   // paper background
    Bitmap icon = Draw(inner, false);
    g.DrawImage(icon, (size - inner) / 2, (size - inner) / 2, inner, inner);
    icon.Dispose();
    g.Dispose();
    return bmp;
  }
...
$icon = [CapAssets]::Draw(1024, $false)
$icon.Save("$assetsDir\icon.png", [System.Drawing.Imaging.ImageFormat]::Png)
$icon.Dispose()
$splash = [CapAssets]::Splash(2732, 900)
$splash.Save("$assetsDir\splash.png", [System.Drawing.Imaging.ImageFormat]::Png)
$splash.Dispose()
Write-Host "cap assets done: $assetsDir"
